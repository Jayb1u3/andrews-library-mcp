...
| `save_work` | Save open-access research full text locally (DOI→Unpaywall/OpenAlex, Digital Commons, or any open PDF URL) |
...
- `save_work` writes files only to `~/.hermes/andrews-library/files/` and only for content served as a PDF without authentication. It does not contact Andrews/Locate and never uses EZproxy or any licensed-vendor session. The Unpaywall contact email is configurable and defaults to a placeholder.
...
5. Use `save_work` only for legitimately open content (open-access DOIs, Digital Commons, or any URL serving a PDF without authentication). It saves to `~/.hermes/andrews-library/files/`.
6. Treat availability, hours, and room slots as time-sensitive. State when the tool response was obtained and provide the source URL when present.
7. If a tool returns pagination or a continuation token, disclose that more results exist before deciding the search is exhaustive.
