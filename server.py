...
UNPAYWALL_EMAIL = os.environ.get("ANDREWS_LIBRARY_UNPAYWALL_EMAIL",
                                 "library-contact@example.org")
...
def _download_pdf(url, save_as=None, overwrite=False, referer=None, cookies=None):
    """Stream a PDF into a .part file and atomically rename on completion.

    Safeguards (mirrors the LearningHub download tool): 100 MB cap, filename
    sanitization, refuse overwrite unless told, and a real Content-Type PDF
    check so a login wall posing as a PDF is rejected with an explanation.
    """
    ...
    with resp:
        ...
        if not name:
            ...
            name = (m.group(1) if m
                        ...
                        urllib.parse.urlparse(resp.geturl()).path.rsplit("/", 1)[-1])
                    or "download.pdf")
...
TOOLS = [
    ...
    {"name": "save_work",
     "description": "Save research full text locally (to ~/.hermes/andrews-library/"
                    "files/) when a legitimately open copy exists: pass `doi` to "
                    "fetch the open-access copy (Unpaywall/OpenAlex), or `url` for "
                    "a Digital Commons record or any open-access PDF link. Digital "
                    "Commons PDFs are CDN-restricted to browsers: the tool then "
                    "returns the direct pdf_url for a one-click browser save. Only "
                    "fetches content that is served as a PDF without authentication.",
     "inputSchema": {"type": "object", "properties": {
