...
class SaveWork(unittest.TestCase):
    ...
    def test_unpaywall_email_is_not_personal(self):
        # Guard against a real personal address leaking into source.
        self.assertNotIn("@gmail.com", server.UNPAYWALL_EMAIL)
        self.assertNotIn("jasonpinilla", server.UNPAYWALL_EMAIL)

    def test_no_oa_doi_returns_factual_result(self):
        with mock.patch.object(server, "resolve_oa_pdf",
                                return_value=(None, None)):
            out = server.tool_save_work({"doi": "10.1234/closed"})
        self.assertFalse(out["saved"])
        self.assertFalse(out["is_open_access"])
        self.assertIn("No legal open-access", out["why"])
        self.assertNotIn("ezproxy_url", out)
        self.assertNotIn("sanctioned", out)

    def test_oa_doi_downloads_via_engine(self):
        with mock.patch.object(server, "resolve_oa_pdf",
                                return_value=("https://oa.example.com/p.pdf",
                                              "unpaywall")), \
             mock.patch.object(server, "_download_pdf",
                               return_value={"saved_to": "/tmp/x.pdf",
                                             "size_bytes": 10}) as dl:
            out = server.tool_save_work({"doi": "10.1234/open"})
        self.assertTrue(out["saved"])
        self.assertEqual(out["source"], "unpaywall")
        self.assertEqual(out["pdf_url"], "https://oa.example.com/p.pdf")
        dl.assert_called_once()

    def test_digitalcommons_extracts_pdf_url(self):
        ...
        def fake_dl(url, **kw):
            ...
            return {"saved_to": "/tmp/d.pdf", "size_bytes": 5}

        with mock.patch.object(server, "http",
                               fake_http_factory([("digitalcommons", 200,
                                                    {"set-cookie": "sid=1"},
                                                    page)])), \
             mock.patch.object(server, "_download_pdf", fake_dl):
            out = server.tool_save_work(
                {"url": "https://digitalcommons.andrews.edu/record/1"})
        self.assertTrue(out["saved"])
        self.assertEqual(out["source"], "digitalcommons")
        self.assertIn("viewcontent.cgi", out["pdf_url"])
        # page cookies + referer were carried into the download attempt
        self.assertEqual(captured["cookies"], "sid=1")
        self.assertIn("digitalcommons.andrews.edu", captured["referer"])

    def test_digitalcommons_403_returns_link_not_error(self):
        page = ('<meta name="citation_pdf_url" content='
                '"https://digitalcommons.andrews.edu/cgi/viewcontent.cgi'
                '?article=1"/>')

        def fake_dl(*a, **k):
            raise server.ToolError("403 bot wall")
        ...
        with mock.patch.object(server, "http",
            ...
            out = server.tool_save_work(
                {"url": "https://digitalcommons.andrews.edu/record/1"})
        self.assertFalse(out["saved"])
        self.assertIn("pdf_url", out)
        self.assertIn("browser", out["how"])

    def test_direct_open_pdf_downloads(self):
        with mock.patch.object(server, "_download_pdf",
                               return_value={"saved_to": "/tmp/o.pdf",
                                             "size_bytes": 7}) as dl:
            out = server.tool_save_work({"url": "https://example.com/a.pdf"})
        self.assertTrue(out["saved"])
        self.assertEqual(out["source"], "direct-open")
        dl.assert_called_once()
